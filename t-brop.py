...
from tbrop.collection import GadgetsCollection
...
group.add_argument(
    "-b", "--bytes", metavar="N", type=int, help="search gadgets up to N bytes"
)
group.add_argument(
    "-c",
    "--cost",
    metavar="N",
    type=int,
    help="search gadgets up to a cost of N (default)",
)
group.add_argument(
    "-i", "--instr", metavar="N", type=int, help="search gadgets up to N instructions"
)
...
if __name__ == "__main__":
    ...
    gdgtCollection = GadgetsCollection("x64", bytearray(sctext.content))
    ...
    print("\nGathering gadgets")
    gdgtCollection.collect(callback=callback, context=context)
